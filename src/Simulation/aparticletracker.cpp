...
bool AParticleTracker::TrackParticlesOnStack(int eventId)
{
    ...
    while ( !ParticleStack.empty() )
    {
        ...
        for (int i=0; i<3; i++)
        {
            r[i] = thisParticle->r[i];
            v[i] = thisParticle->v[i];
        }
        NormalizeVector(v); //normalization of the staring vector
        double energy = thisParticle->energy;     //particle energy
        double time   = thisParticle->time;       //particle creation time - not modified!

        const int ParticleId = thisParticle->Id; //id of the particle
        ...
        if (navigator->IsOutside())
        {
            if (SimSet->fLogsStat)
            {
                ATrackingStepData * step = new ATrackingStepData(thisParticle->r, thisParticle->time, thisParticle->energy, 0, "O");
                thisParticleRecord->addStep(step);
            }
        ...
        bool bBuildThisTrack = BuildTracks; // ***!!! need two variables?
        if (bBuildThisTrack)
        {
