...
// ----- older model - V1 -----
double PhScatClaudioModelV1::GnFunc(double cost)
{
  return 2/(1+sqrt(1+sigma_alpha*sigma_alpha*(1-cost*cost)/(cost*cost)));
}

double PhScatClaudioModelV1::SlopeAngle(double random_num)
{
  if (SlopeDistribution == trowbridgereitz)
    return atan(sigma_alpha*sqrt(random_num)/(sqrt(1.0-random_num)));
  if (SlopeDistribution == cooktorrance)
    return atan(sqrt(-sigma_alpha*sigma_alpha*log(1.0-random_num)));
  if (SlopeDistribution == bivariatecauchy)
    return atan(sigma_alpha*sqrt(random_num*(2.0-random_num)));
  return 0;
}

AOpticalOverride::OpticalOverrideResultEnum PhScatClaudioModelV1::calculate(TRandom2* RandGen, APhoton *Photon, const double *NormalVector)
{
  TVector3 K(Photon->v);                // photon direction
  //qDebug() << "Photon direction (i,j,k):"<<K.x()<<K.y()<<K.z();
  //qDebug() << "iWave:"<<iWave;
...
//  qDebug() << "gni:"<<gni;
  ...
  do
    {
      if ( RandGen->Rndm() < lambda || sigma_alpha == 0)
      ...
      else
        {
          ...
          costr = 2*costl*CosAlpha_WRM-costi; //angle between the global normal and the viewing direction
          gnr  = GnFunc(costr);
  ...
  //The polarization of the incident photon
  //G4ThreeVector A_trans = OldMomentum.cross(theScatNormal); //Measuring the Polarization
  //A_trans = A_trans.unit();

  double E1_perp = sqrt(2.0)/2.0;//OldPolarization * A_trans;
  //G4ThreeVector E1pp    = E1_perp * A_trans;
  //G4ThreeVector E1pl    = OldPolarization - E1pp;
  double E1_parl = E1_perp;//E1pl.mag();
  ...
  if (RandGen->Rndm()<Amp_tot)
  ...
  else
    {
      ...
      do
        {
          ...
          costr = GlobalNormal*K;
          double sintr = sqrt(1.0-costr*costr);
          if (costr > 0)
              gnd = GnFunc(costr);
          else
              gnd = 0;

          double sintinter = Rindex1*sintr/Rindex2;
          if(sintinter<1)
            {
              double costiter = std::sqrt(1.-sintinter*sintinter);
              double Fres_Term  = 1 - Fresnel(sqrt(2.0)/2, sqrt(2.0)/2, costiter, Rindex2, Rindex1);
              flag_ready = 1;
              if (RandGen->Rndm() > albedo*Fres_Term)
                {
                  Status = Absorption;
                  Photon->SimStat->OverrideClaudioAbs++;
                  return Absorbed;
      ...
      while (sigma_alpha> 0 && RandGen->Rndm() > gnd || flag_ready == 0);
...
QString PhScatClaudioModelV1::getReportLine()
{
  return PhScatClaudioModel::getReportLine()+"_v1";
}
